...
class InstagramScraper(object):
    ...
    def __scrape_query(
        self, media_generator, executor=concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS)
    ):
            ...
            for value in self.usernames:
                ...
                for item in tqdm.tqdm(
                    ...
                    if self.filter_locations:
                        if (
                            item.get("location") is None
                            or self.get_key_from_value(self.filter_locations, item["location"].get("id")) is None
                        ):
                            continue
                    if (
                        (item["is_video"] is False and "image" in self.media_types)
                        or (item["is_video"] is True and "video" in self.media_types)
                        ...
                        if future.exception() is not None:
                            self.logger.warning(
                                "Media for {0} at {1} generated an exception: {2}".format(
                                    value, item["urls"], future.exception()
                                )
                            )
                        else:
    ...
    def scrape(self):
        ...
        try:
            ...
            last_timestamp = self.get_last_scraped_timestamp(username)
            self.last_scraped_filemtime = last_timestamp
    ...
    def get_media(self, user):
        ...
        items = [i for i in self.query_media_gen(user) if self.is_new_media(i)]
        return items
    ...
    def update_ig_gis_header(self, params):
        self.session.headers.update({"x-instagram-gis": self.get_ig_gis(self.rhx_gis, params)})
                        ...
                        while True:
                            ...
                            try:
                                ...
                                with self.session.get(
                                    url, cookies=self.cookies, headers=headers, stream=True, timeout=CONNECT_TIMEOUT
                                ) as response:
                                    ...
                                    if response.status_code == 206:
                                        try:
                                            match = re.match(
                                                r"bytes (?P<first>\d+)-(?P<last>\d+)/(?P<size>\d+)",
    ...
    def search_locations(self):
        ...
        for item in sorted_places[0:5]:
            place = item["place"]
            print(
                "location-id: {0}, title: {1}, subtitle: {2}, city: {3}, lat: {4}, lng: {5}".format(
                    place["location"]["pk"],
                    place["title"],
                    place["subtitle"],
                    place["location"]["city"],
...
def main():
    ...
    parser.add_argument("username", help="Instagram user(s) to scrape", nargs="*", default="kolja.es")
    parser.add_argument("--destination", "-d", default="/home/david/Downloads/foxglove", help="Download destination")
    ...
    parser.add_argument(
        "--followings-output", "--followings_output", help="Output followings-input to file in destination"
    )
    ...
    parser.add_argument(
        "--proxies", default={}, help="Enable use of proxies, add a valid JSON with http or/and https urls."
    )
    parser.add_argument(
        "--include-location",
        "--include_location",
    ...
    parser.add_argument(
        "--filter-location-file", default=None, type=str, help="file containing list of locations to filter query by"
    )
    ...
    parser.add_argument(
        "--no-check-certificate", action="store_true", default=False, help="Do not use ssl on transaction"
    )
    parser.add_argument(
        "--interactive", "-i", action="store_true", default=False, help="Enable interactive login challenge solving"
    )
    parser.add_argument(
        "--retry-forever",
        action="store_true",
    ...
    parser.add_argument(
        "--log_destination", "-l", type=str, default="", help="destination folder for the instagram-scraper.log file"
    )
